...
import { ColabInterface } from 'interfaces/colab';
import { getColabs } from 'apiSdk/colabs';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'team-member', 'guest-user', 'administrator', 'room-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ColabInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/colabs` : null),
    () => getColabs({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Colabs',
      path: isTenantUser ? `/colabs/view/${data?.[0]?.id}` : '/colabs',
      entity: 'colab',
    ...
    { name: 'Rooms', path: '/rooms', entity: 'room', service: AccessServiceEnum.PROJECT, icon: FiHome },
    {
      name: 'Team Members',
      path: '/team-members',
      entity: 'team_member',
