...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'Room Booking.create': {
    roles: ['business-owner', 'team-member', 'administrator', 'room-manager'],
    key: 'room_booking',
    tenantPath: [],
    userPath: [],
  },
  'Room Cancellation.delete': {
    roles: ['business-owner', 'team-member', 'administrator', 'room-manager'],
    key: 'room_cancellation',
    tenantPath: [],
    userPath: [],
  },
...
const tenantRoles: string[] = ['business-owner', 'team-member', 'guest-user', 'administrator', 'room-manager'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.colab.findFirst({
